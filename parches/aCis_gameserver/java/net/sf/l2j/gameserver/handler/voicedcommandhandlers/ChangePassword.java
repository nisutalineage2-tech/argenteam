		...
		if (command.equalsIgnoreCase("changepass") && target != null)
		{
			...
			try
			{
				...
				if (!(curpass == null || newpass == null || repeatnewpass == null))
				{
					...
					byte[] raw = curpass.getBytes(StandardCharsets.UTF_8);
					String curpass64 = Base64.getEncoder().encodeToString(md.digest(raw));
					...
					if (curpass64.equals(pass))
					{
						raw = newpass.getBytes(StandardCharsets.UTF_8);
						String newpass64 = Base64.getEncoder().encodeToString(md.digest(raw));
						
						PreparedStatement ps = con.prepareStatement("UPDATE accounts SET password=? WHERE login=?");
						ps.setString(1, newpass64);
