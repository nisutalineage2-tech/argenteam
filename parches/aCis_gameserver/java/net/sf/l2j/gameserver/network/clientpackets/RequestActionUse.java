	...
	protected void runImpl()
	{
		...
		switch (_actionId)
		{
			...
			case 22: // Attack (pet attack)
				...
				if (!activeChar.isInSiege() && !activeChar.isInOlympiadMode())
				{
					boolean cond = true;
					...
					{
						player = ((L2PetInstance) target).getOwner();
					}
					if (player != null && player != activeChar)
					{
						if (activeChar.getFactionId() == player.getFactionId() && player.getFactionId() > 0)
						{
							cond = false;
						}
						else if (activeChar.getFactionId() == 0 || player.getFactionId() == 0)
						{
							cond = false;
						}
					}
