 ...
 * @author rapfersan92
 */
public class L2MultiZoneHunter extends L2ZoneType
{
	...
	private static List<Location> _hunter2 = new ArrayList<>();
	private static List<Location> _hunter1 = new ArrayList<>();
	private static List<Integer> _restrictedItems = new ArrayList<>();
	private static List<Integer> _restrictedSkills = new ArrayList<>();
	
	public L2MultiZoneHunter(int id)
	{
		super(id);
	...
	public void setParameter(String name, String value)
	{
		...
		else if (name.equals("hunter1"))
		{
			String[] property = value.split(";");
			for (String data : property)
			{
				String[] coordinates = data.split(",");
				_hunter1.add(new Location(Integer.parseInt(coordinates[0]), Integer.parseInt(coordinates[1]), Integer.parseInt(coordinates[2])));
			}
		}
		else if (name.equals("hunter2"))
		{
			String[] property = value.split(";");
			for (String data : property)
			{
				String[] coordinates = data.split(",");
				_hunter2.add(new Location(Integer.parseInt(coordinates[0]), Integer.parseInt(coordinates[1]), Integer.parseInt(coordinates[2])));
			}
		}
	...
	private static void respawnCharacter(L2Character character)
	{
		...
		if (activeChar.getFactionId() == 1)
		{
			...
			character.teleToLocation(Rnd.get(_hunter1), 20);
		}
		...
		{
			character.doRevive(_revivePower);
			character.teleToLocation(Rnd.get(_hunter2), 20);
		}
		}
	}
