 ...
 * @author rapfersan92
 */
public class L2MultiZoneRuins extends L2ZoneType
{
	...
	private static List<Location> _ruins2 = new ArrayList<>();
	private static List<Location> _ruins1 = new ArrayList<>();
	private static List<Integer> _restrictedItems = new ArrayList<>();
	private static List<Integer> _restrictedSkills = new ArrayList<>();
	
	public L2MultiZoneRuins(int id)
	{
		super(id);
	...
	public void setParameter(String name, String value)
	{
		...
		else if (name.equals("ruins1"))
		{
			String[] property = value.split(";");
			for (String data : property)
			{
				String[] coordinates = data.split(",");
				_ruins1.add(new Location(Integer.parseInt(coordinates[0]), Integer.parseInt(coordinates[1]), Integer.parseInt(coordinates[2])));
			}
		}
		else if (name.equals("ruins2"))
		{
			String[] property = value.split(";");
			for (String data : property)
			{
				String[] coordinates = data.split(",");
				_ruins2.add(new Location(Integer.parseInt(coordinates[0]), Integer.parseInt(coordinates[1]), Integer.parseInt(coordinates[2])));
			}
		}
	...
	private static void respawnCharacter(L2Character character)
	{
		...
		if (activeChar.getFactionId() == 1)
		{
			...
			character.teleToLocation(Rnd.get(_ruins1), 20);
		}
		...
		{
			character.doRevive(_revivePower);
			character.teleToLocation(Rnd.get(_ruins2), 20);
		}
		}
	}
