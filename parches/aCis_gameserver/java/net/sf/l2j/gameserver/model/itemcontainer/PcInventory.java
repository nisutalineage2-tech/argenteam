...
public class PcInventory extends Inventory
{
	...
	public ItemInstance addItem(String process, int itemId, long countL, L2Object reference, boolean sendMessage)
	{
		...
		if (count > 0)
		{
			...
			if (sendMessage && ((!_owner.isCastingNow() && item.getItemType() == EtcItemType.HERB) || item.getItemType() != EtcItemType.HERB))
			{
				if (count > 1)
				{
					if (process.equalsIgnoreCase("Sweep") || process.equalsIgnoreCase("Quest"))
						_owner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(itemId).addItemNumber(count));
					else
						_owner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_PICKED_UP_A_S1_S2).addItemName(itemId).addItemNumber(count));
				}
				else
				{
					if (process.equalsIgnoreCase("Sweep") || process.equalsIgnoreCase("Quest"))
						_owner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_ITEM_S1).addItemName(itemId));
					else
						_owner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_PICKED_UP_S1).addItemName(itemId));
				}
			}
			...
			if (item.getItemType() == EtcItemType.HERB)
			{
				...
				if (handler != null)
					handler.useItem(_owner, herb, false);
			}
			else
			{
				// Add the item to inventory
				final ItemInstance createdItem = addItem(process, itemId, count, _owner, reference);
				
				// Cursed Weapon
				if (CursedWeaponsManager.getInstance().isCursed(createdItem.getItemId()))
					CursedWeaponsManager.getInstance().activate(_owner, createdItem);
				// If you pickup arrows and a bow is equipped, try to equip them if no arrows is currently equipped.
				else if (item.getItemType() == EtcItemType.ARROW && _owner.getAttackType() == WeaponType.BOW && getPaperdollItem(Inventory.PAPERDOLL_LHAND) == null)
					_owner.checkAndEquipArrows();
