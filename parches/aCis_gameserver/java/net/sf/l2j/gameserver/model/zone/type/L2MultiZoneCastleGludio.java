 ...
 * @author rapfersan92
 */
public class L2MultiZoneCastleGludio extends L2ZoneType
{
	...
	private static List<Location> _castlegludio2 = new ArrayList<>();
	private static List<Location> _castlegludio1 = new ArrayList<>();
	private static List<Integer> _restrictedItems = new ArrayList<>();
	private static List<Integer> _restrictedSkills = new ArrayList<>();
	
	public L2MultiZoneCastleGludio(int id)
	{
		super(id);
	...
	public void setParameter(String name, String value)
	{
		...
		else if (name.equals("castlegludio1"))
		{
			String[] property = value.split(";");
			for (String data : property)
			{
				String[] coordinates = data.split(",");
				_castlegludio1.add(new Location(Integer.parseInt(coordinates[0]), Integer.parseInt(coordinates[1]), Integer.parseInt(coordinates[2])));
			}
		}
		else if (name.equals("castlegludio2"))
		{
			String[] property = value.split(";");
			for (String data : property)
			{
				String[] coordinates = data.split(",");
				_castlegludio2.add(new Location(Integer.parseInt(coordinates[0]), Integer.parseInt(coordinates[1]), Integer.parseInt(coordinates[2])));
			}
		}
	...
	protected void onEnter(L2Character character)
	{
		...
		if (character instanceof L2PcInstance || character instanceof L2PetInstance)
		{
			...
			player.sendPacket(new ExShowScreenMessage("You have entered a Range zone.", 5000));
	...
	protected void onExit(L2Character character)
	{
		...
		{
			final L2PcInstance player = (L2PcInstance) character;
			player.sendPacket(new ExShowScreenMessage("You have left range zone.", 5000));
	...
	private static void respawnCharacter(L2Character character)
	{
		...
		if (activeChar.getFactionId() == 1)
		{
			...
			character.teleToLocation(Rnd.get(_castlegludio1), 20);
		}
		...
		{
			character.doRevive(_revivePower);
			character.teleToLocation(Rnd.get(_castlegludio2), 20);
		}
		}
	}
