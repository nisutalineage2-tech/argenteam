...
import net.sf.l2j.gameserver.custom.protection.AntiPlayer;
...
public class GameServer
{
	...
	public GameServer() throws Exception
	{
		...
		StringUtil.printSection("ExGuard");
		ExGuard.getInstance();
		...
		StringUtil.printSection("Faction Engine");
		FactionMaps.loadCurrentMap();
		L2TpFlagInstance.spawnFlags();
		
		StringUtil.printSection("Chaos Event");
		if (Config.FACTION_ENABLE_CHAOS_EVENT)
		{
			ChaosEvent.uzkrautiEventa();
			_log.config("- Currently enabled.");
		}
		else
		{
			_log.config("- Currently disabled.");
		}
